...
class LocationSpellCorrector:
    ...
    def __init__(self):
        self._ref_set: set = set(_CHENNAI_REFERENCE_LOCALITIES)
    ...
    def correct(
        ...
        # Pass 2 — difflib close matches on normalised strings
        norm_list   = [n for _, n in corpus]
        canon_list  = [c for c, _ in corpus]
        close       = difflib.get_close_matches(
            query_norm, norm_list, n=1, cutoff=self.CUTOFF
        )
        if close:
            idx       = norm_list.index(close[0])
            canonical = canon_list[idx]
            score     = difflib.SequenceMatcher(None, query_norm, close[0]).ratio()
            logger.info(
                f"[SpellCorrector] '{query}' → '{canonical}' "
                f"(score={score:.2f}, pass=2-difflib)"
            )
        ...
        if len(tokens) > 1:
            ...
            for tok in tokens:
                if len(tok) < self.MIN_LENGTH:
                    ...
                    continue
                tok_close = difflib.get_close_matches(
                    tok, norm_list, n=1, cutoff=self.CUTOFF
                )
                if tok_close:
                    corrected_tokens.append(tok_close[0])
                    if tok_close[0] != tok:
                        changed = True
                else:
                    corrected_tokens.append(tok)
            if changed:
                ...
                full_close = difflib.get_close_matches(
                    reconstructed, norm_list, n=1, cutoff=0.55
                )
                if full_close:
                    idx       = norm_list.index(full_close[0])
                    canonical = canon_list[idx]
                    logger.info(
    ...
    def _build_corpus(
        ...
        # Reference list (supplemental)
        for loc in _CHENNAI_REFERENCE_LOCALITIES:

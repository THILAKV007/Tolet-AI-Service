...
class PropertyDBService:
    ...
    def get_all_localities_with_city(self) -> list:
        """
        Returns deduplicated {"locality", "city", "state"} triples from the DB,
        instead of a flat list of locality strings. Needed so geo expansion can
        disambiguate the same locality name appearing in two different cities
        (e.g. "Anna Nagar" exists in both Chennai and Madurai) — a flat
        get_all_localities() list loses that information entirely.
        """
        if self.collection is None:
            return []
        try:
            ...
            for doc in docs:
                ...
                if not locality:
                    continue
                key = (locality.lower(), city.lower(), state.lower())
                if key in seen:
                    continue
                seen.add(key)
                result.append({"locality": locality, "city": city, "state": state})
            result.sort(key=lambda r: (r["locality"], r["city"]))

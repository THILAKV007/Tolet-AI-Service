...
sample_properties = [
    {
        ...
        # NOTE: "type" is the field property_db_service.py actually reads for
        # posted_by / owner_type filtering (doc.get("type")). isBrokerExcuse
        # alone does NOT drive that logic — keep both in sync so local seed
        # data exercises the same code path as production data.
        "type":               "direct_owner",
        "isBrokerExcuse":     True,
        "petsAllowed":        "No",
        "ownerAtPlace":       "No",
...
# All sample docs must be "approved" — every real query filters on
# status="approved" (see property_db_service.py::_apply_common_filters).
# Seed data without this would silently return zero results for every
# search, which would be a confusing false negative during local testing.
for _p in sample_properties:
    _p.setdefault("status", "approved")


# ===================================
# Indexes
# property_db_service.py runs an $or regex query across locality/city/state
# on every single search, plus a status filter on every query and a type
# filter on owner-type searches. Without indexes these are full collection
# scans — fine at 29 docs, a real bottleneck at production scale.
# ===================================
collection.create_index([("locality", 1)])
...
print("Indexes ensured: locality, city, state, status, type, propertyType")
